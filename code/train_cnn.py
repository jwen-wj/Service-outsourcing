...
x_train, y_train = DH.process_file(DH.TextConfig.trainFileCsv,word_to_id, cat_to_id,num_classes,DH.TextConfig.seq_length)
x_val, y_val = DH.process_file(DH.TextConfig.val_filename,word_to_id, cat_to_id,num_classes,DH.TextConfig.seq_length)
pad_seq_len = len(x_train[0])

num_epochs = 40
batch_size = 256
checkpoint_every = int(int(math.ceil(len(x_train) / batch_size)) * (num_epochs/10))
...
tf.flags.DEFINE_integer("vocab_size", vocab_size, "vocabulary size (default: 5000)")
...
tf.flags.DEFINE_integer("num_classes", len(categories), "Number of labels (depends on the task)")
...
tf.flags.DEFINE_integer("num_epochs", num_epochs, "Number of training epochs (default: 100)")
tf.flags.DEFINE_integer("evaluate_every", 800, "Evaluate model on dev set after this many steps (default: 5000)")
...
tf.flags.DEFINE_integer("checkpoint_every", checkpoint_every, "Save model after this many steps (default: 1000)")
...
logger = DH.logger_fn('tflog', 'logs/training-{0}.log'.format( time.strftime("%Y-%m-%d-%H_%M_%S",time.localtime(time.time()))))
